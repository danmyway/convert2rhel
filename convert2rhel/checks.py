...
def is_loaded_kernel_latest():
    ...
    if not packages:
        ...
        if any(envvar in os.environ for envvar in allow_older_envvar_names):
            ...
            logger.critical(
                "Could not find any %s packages in repositories to compare against the loaded kernel."
                " We are unable to determine if the running kernel is the latest available. The conversion will"
                " install the latest RHEL kernel so it is important to check that you can boot into the equivalent"
                " kernel from your current distribution before converting. Check to see if you have the standard vendor"
                " repositories enabled, then re-run the conversion. If you wish to ignore this check, you can set the"
                " environment variable to 'CONVERT2RHEL_SKIP_KERNEL_CURRENCY_CHECK=1' to skip this check."
                % (package_to_check)
            )
        return
