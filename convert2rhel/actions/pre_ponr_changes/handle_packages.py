...
class ListThirdPartyPackages(actions.Action):
    ...
    def run(self):
        ...
        if third_party_pkgs:
            ...
            # Perform skip if the user used the environment variable.
            if third_party_package_skip:
                skip_message = (
...
class RemoveExcludedPackages(actions.Action):
    ...
    def run(self):
        ...
        if pkgs_not_removed:
            pkg_names = [pkghandler.get_pkg_nevra(pkg, include_zero_epoch=True) for pkg in pkgs_not_removed]
            self.add_message(
                level="WARNING",
